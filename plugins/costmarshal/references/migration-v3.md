...
external certification. Reconfigure the gateway boundary with:

- `--deployment-commit <40-hex>`;
- `--release-version v4.2.0`;
- `--gateway-image name@sha256:<64-hex>`;
- `--allowed-signers-sha256 sha256:<64-hex>`; and
- one or more `--signer-identity`.

Register each external report as an accepted project Artifact with kind
`production-evidence`. Create the canonical claim with
`scripts/costmarshal_production_certification.py create`, sign its exact bytes
outside CostMarshal with `ssh-keygen -Y sign`, and expose the manifest,
signature, and reviewed `allowed_signers` file to the scheduler through the
three `COSTMARSHAL_PRODUCTION_*_FILE` variables. Claims expire after at most
seven days and must be renewed after any bound commit, policy, image, boundary,
or evidence receipt changes.
...
live Responses call. DeepSeek and Kimi use
`configure-provider --via-production-gateway`; the emitted catalog row remains
non-dispatchable until the enforced gateway is externally certified. Replace a profile explicitly,
