...
## v4.0 trust boundaries

CostMarshal v4.0's OCI controls isolate worker processes from
...
attempt/provider/budget/expiry-scoped capability. v4.0 defines a secret-free
external Broker/Provider Proxy configuration and evidence contract, but does
not include the attested workload-identity runtime adapter.
`production-status` therefore remains blocked, and an enforced boundary blocks
dispatch. The `provider-proxy` network validates controlled egress topology; it
does not by itself remove the raw credential from the worker.
