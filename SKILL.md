...
With complete reviewed prices and token estimates, exhaustively compare every safe monotonic provider subchain, including early-stop and tier-skip plans, using expected cost per leader-accepted result. Reject more than 16 enabled compatible providers in one tier. Enforce the task `--min-success-probability` when supplied, otherwise freeze the optional project `default_min_success_probability` into a new auto-routed task. Without a positive success floor, if the complete chain across all available safe tiers at or above the task floor has fewer than 10 exact task/profile/conditional-lineage observations at a continuation, seal the most cost-efficient such chain in `conditional-evidence-bootstrap` mode. This is low→medium→high only for a complete low-floor catalog, medium→high for a medium floor, and no bootstrap for a single-step high floor. Do not invent a continuation probability: every successor still needs an explicit leader rejection, so leader acceptance stops further spend. After those lineages mature, return to exhaustive early-stop/tier-skip optimization. An explicit provider stays pinned; an explicit tier with complete pricing and token estimates ranks same-tier peers by conservative cost per accepted result, using priority only as a later tie-breaker. Before the first dispatch, bind each selected step to its price basis and exact-byte provider profile identity, then reserve the sum of all step estimates in a task-level admission envelope; never use probability-weighted expected cost as the reservation. Continuations consume that envelope and immutable profile snapshot without double counting or re-reading mutable source config, and drift fails closed. Without complete economic inputs, choose the minimum safe available tier. Explain the fallback; do not manufacture a price.
